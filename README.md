-- LocalScript: полный UI + парольный ввод + занавес + ESP (bbox + линия + инфо) + кнопки Teleport / Toggle ESP / Destroy GUI
-- Поместить в StarterPlayerScripts
--
-- Требует в ReplicatedStorage:
--   RemoteEvent "AdminEvent"   -- сервер -> клиент: AdminEvent:FireClient(player, "esp_update", {userId, ...})
--   RemoteEvent "AdminRequest" -- клиент -> сервер: AdminRequest:FireServer(action, param)
--
-- Серверная часть обязана проверять права и выполнять действия (isAdmin). Клиентская проверка пароля
-- служит только UX-целям и легко обходится, поэтому главное — безопасность на сервере.
-- Пароль по умолчанию (локальная проверка): "TPSC9"
-- ==============================================================================

local UI_PASSWORD = "TPSC9"

-- Внешний вид
local BG_COLOR = Color3.fromRGB(43, 43, 43)       -- тёмно-серый фон для баров
local TEXT_COLOR = Color3.fromRGB(255, 255, 255)  -- белый текст
local ACCENT_COLOR = Color3.fromRGB(0, 200, 255)  -- акцент (рамки)
local LINE_COLOR = Color3.fromRGB(255, 100, 100)  -- цвет линии до цели
...
local function createPasswordModal()
    ...
    ok.MouseButton1Click:Connect(function()
        if box.Text == UI_PASSWORD then
            ...
            modal:Destroy()
            passwordModal = nil
            -- play curtain animation и показать main UI
...
-- ========== Main horizontal bar (список игроков и кнопки) ==========
local mainBar
local playersScrollingFrame
local selectedUserId = nil
local playerButtonRefs = {} -- userId -> button

local function createMainBar()
    local bar = Instance.new("Frame")
    bar.Name = "MainBar"
    bar.Size = UDim2.new(1, -40, 0, 64)
    bar.Position = UDim2.new(0, 20, 0, 56) -- под заголовком
    ...
    -- ScrollingFrame для списка игроков (горизонтально)
    ...
    -- кнопки: Teleport, Toggle ESP, Destroy
    ...
    destroyBtn.MouseButton1Click:Connect(function()
        -- попытаемся очистить серверную ESP и уничтожить GUI
...
-- Обновление списка игроков в ScrollingFrame
...
-- ========== ESP реализация (bbox + линия + инфо) ==========
local tracked = {} -- userId -> table
...
-- Обработка событий от сервера (AdminEvent)
...
-- ========== Curtain animation + show main ==========

function playCurtainAndShowMain()
    -- опускаем занавес вниз (видим), затем поднимаем - эффект "занавес/появление"
    ...
    -- немного паузы, затем показываем mainBar и поднимаем занавес вверх
...
function cleanupAndDestroy()
    -- остановим рендер и оборвём связи
    stopRenderLoop()
    if adminEventConn then adminEventConn:Disconnect(); adminEventConn = nil end
    -- удалим GUI
    if screenGui and screenGui.Parent then screenGui:Destroy() end
    -- отключим действия (если использовались)
end

-- ========== Инициализация ==========
-- создаём password modal и показываем (см. createPasswordModal)
createPasswordModal()

-- Если пользователь уже ввёл пароль и modal не нужен (возможность быстрого теста),
-- можно вызвать playCurtainAndShowMain() вручную в коде.

-- Также добавим хоткей закрытия GUI для удобства (Ctrl + M например) — локальное действие
local function toggleGuiAction(actionName, inputState, inputObject)
    if inputState ~= Enum.UserInputState.Begin then return end
    -- простая проверка — если GUI существует, уничтожаем; иначе снова показываем пароль
    if screenGui and screenGui.Parent then
        ...
        -- повторно создать (редкий случай)
        screenGui.Parent = PlayerGui
        createPasswordModal()
    end
end
...
-- ======================================================================
-- Конец скрипта. Напоминание:
-- - Этот скрипт рисует GUI и ESP только на клиенте и отправляет команды на сервер через AdminRequest.
-- - Не забудь поставить серверную логику (AdminRequest/ServerScript), которая:
--     * Проверяет isAdmin(player)
--     * Выполняет телепорт (teleport)
--     * Управляет набором целей ESP и рассылает AdminEvent:FireClient(admin, "esp_update", targets)
-- - Если хочешь, могу прислать/дописать серверный Script (ServerScriptService) и ModuleScript для списка админов.
-- ======================================================================
